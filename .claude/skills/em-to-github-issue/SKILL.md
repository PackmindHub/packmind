...
- Curated spec: `tmp/em-curations/{filename}` *(only when a curated spec was used)*
...
**Title**: short, imperative, ≤70 chars. Take the *goal* part of the user story ("I want X") and rewrite it as an action — e.g. story "As a buyer, I want to apply a loyalty discount at checkout, so that…" becomes title "Apply loyalty discount at checkout". Avoid trailing periods. Don't prefix with `[FEATURE]` or similar tags unless the repo's other issues do.
