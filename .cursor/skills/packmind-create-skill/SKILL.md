...
Skills are modular, self-contained packages that extend Cursor's capabilities by providing
specialized knowledge, workflows, and tools. Think of them as "onboarding guides" for specific
domains or tasks—they transform Cursor from a general-purpose agent into a specialized agent
...
**Metadata Quality:** The `name` and `description` in YAML frontmatter determine when Cursor will use the skill. Be specific about what the skill does and when to use it. Use the third-person (e.g. "This skill should be used when..." instead of "Use this skill when...").
...
- **Note**: Scripts may still need to be read by Cursor for patching or environment-specific adjustments

##### References (`references/`)

Documentation and reference material intended to be loaded as needed into context to inform Cursor's process and thinking.

- **When to include**: For documentation that Cursor should reference while working
- **Examples**: `references/finance.md` for financial schemas, `references/mnda.md` for company NDA template, `references/policies.md` for company policies, `references/api_docs.md` for API specifications
- **Use cases**: Database schemas, API documentation, domain knowledge, company policies, detailed workflow guides
- **Benefits**: Keeps SKILL.md lean, loaded only when Cursor determines it's needed
...
- **Benefits**: Separates output resources from documentation, enables Cursor to use files without loading them into context
...
3. **Bundled resources** - As needed by Cursor (Unlimited\*)
...
When editing the (newly-generated or existing) skill, remember that the skill is being created for another instance of Cursor to use. Focus on including information that would be beneficial and non-obvious to Cursor. Consider what procedural knowledge, domain-specific details, or reusable assets would help another Cursor instance execute these tasks more effectively.
...
3. In practice, how should Cursor use the skill? All reusable skill contents developed above should be referenced so that Cursor knows how to use them.
