...
export type StagePathStatus = 'staged' | 'skipped' | 'failed';

export type StagePathOutcome = {
...
export type StageSinglePathDependencies = {
...
/**
 * Stages exactly one path into the local playbook.
 *
 * Returns an outcome describing what happened and never terminates the
 * process — the caller owns the single `exit` and the error reporting, so the
 * same logic can be reused when staging a batch of paths.
 */
export async function stageSinglePath(
  deps: StageSinglePathDependencies,
): Promise<StagePathOutcome> {
  const {
    packmindCliHexa,
    filePath,
    spaceSlug,
