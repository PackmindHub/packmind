...
### Direct Source File Invocation (GitLab Duo, Junie, AGENTS.md)

For AI assistants without native command rendering (**GitLab Duo**, **Junie**, **AGENTS.md**, and **Codex**), commands are not rendered in agent-specific files. Instead, you must reference the source command file directly from the `.packmind/commands/` folder:
...
<Note>
  ...
  Duo, OpenCode, and Codex. Other AI assistants do not support skill rendering.
</Note>
...
4. For GitLab Duo, Junie, or AGENTS.md: Commands are not rendered separately—use the files directly from `.packmind/commands/`
...
1. Skills are only rendered for GitHub Copilot, Claude Code, Cursor, GitLab Duo, OpenCode, and Codex
