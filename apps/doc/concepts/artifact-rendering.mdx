---
title: 'Understanding Where Your Artifacts Appear'
---
...
Your organization administrator controls which AI assistants receive updates in **Settings** → **Distribution** → **Rendering**. See [Manage AI Agent Rendering](/administration/manage-ai-agents) for details.
...
| OpenCode       | `AGENTS.md` file (in repository root) |
| Codex          | `AGENTS.md` file (in repository root) |
| Kiro           | `.kiro/steering/` folder              |

<Tip>
  Standards work automatically—you don't need to do anything special to activate
  them. Your AI assistant reads them and applies the guidelines when generating
  code.
</Tip>

### How Single-File Rendering Works

Some AI assistants (**AGENTS.md**, **GitLab Duo**, **Junie**, **OpenCode**, **Codex**) use a single shared file rather than a dedicated folder for standards. Because you may want to add your own instructions to these files, Packmind uses HTML comment markers to delineate its managed section:

```
<!-- start: Packmind standards -->
... Packmind-managed content ...
<!-- end: Packmind standards -->
```

Packmind only updates the content **between** these markers. You are free to add your own content before or after them — it will be preserved across distributions.

If the markers don't exist yet (for example, in a brand-new file), Packmind appends the section at the end of the file.
...
For AI assistants that support native integration (**Claude Code**, **Cursor**, **GitHub Copilot**, **Continue**, and **Kiro**), these glob patterns are included in the rendered files. The AI assistant will automatically apply the standard only to files matching the specified patterns.
...
<Warning>
  For **Kiro**, the scope is rendered as `inclusion: fileMatch` with a
  `fileMatchPattern` list. The Kiro IDE honours it and applies the standard only
  to matching files. The Kiro **CLI** ignores steering inclusion modes and loads
  every file in `.kiro/steering/`, so scoped standards are not narrowed there.
</Warning>
...
| OpenCode       | `.opencode/commands/` folder | Type `/command-name` in chat                |
| Codex          | Not rendered (use source)    | Execute @.packmind/commands/command-name.md |
| Kiro           | Not rendered (use source)    | Execute @.packmind/commands/command-name.md |

### Using Slash Commands (Claude Code, Cursor, GitHub Copilot, Continue)

For AI assistants with native slash command support (**Claude Code**, **Cursor**, **GitHub Copilot**, and **Continue**), commands are rendered in dedicated folders and can be invoked using slash commands:
...
### Direct Source File Invocation (GitLab Duo, Junie, AGENTS.md, Codex, Kiro)

For AI assistants without native command rendering (**GitLab Duo**, **Junie**, **AGENTS.md**, **Codex**, and **Kiro**), commands are not rendered in agent-specific files. Instead, you must reference the source command file directly from the `.packmind/commands/` folder:
...
| Kiro           | `.kiro/skills/{skill-slug}/SKILL.md`       |
...
<Note>
  Skills are currently available for GitHub Copilot, Claude Code, Cursor, GitLab
  Duo, OpenCode, Codex, and Kiro. Other AI assistants do not support skill
  rendering.
</Note>
...
3. For Claude Code, Cursor, or GitHub Copilot: Verify the AI assistant-specific folder exists (e.g., `.cursor/commands/`)
4. For GitLab Duo, Junie, AGENTS.md, Codex, or Kiro: Commands are not rendered separately—use the files directly from `.packmind/commands/`
...
1. Skills are only rendered for GitHub Copilot, Claude Code, Cursor, GitLab Duo, OpenCode, Codex, and Kiro
2. Check the `.packmind/skills/` folder for the source files
3. For GitHub Copilot: Check `.github/skills/{skill-slug}/SKILL.md`
4. For Claude Code: Check `.claude/skills/{skill-slug}/SKILL.md`
5. For Cursor: Check `.cursor/skills/{skill-slug}/SKILL.md`
6. For GitLab Duo: Check `.gitlab/duo/skills/{skill-slug}/SKILL.md`
7. For Kiro: Check `.kiro/skills/{skill-slug}/SKILL.md`

## Related Documentation

- [Manage AI Agent Rendering](/administration/manage-ai-agents) - Configure which AI assistants receive your artifacts
- [Distribute Artifacts](/governance/distribution) - Learn how to distribute packages to repositories
- [Standards Management](/concepts/standards-management) - Create and manage coding standards
- [Commands Management](/concepts/commands-management) - Create and manage reusable commands
