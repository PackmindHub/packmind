...
2. **Through AI assistants** — Use the `/packmind-update-playbook` skill for AI-guided command creation
...
<Tip>
  ...
  the `/packmind-update-playbook` skill when you want AI assistance in
  structuring commands from your development history.
</Tip>

### Creating Commands with AI Assistants

You can create commands using the `/packmind-update-playbook` command in your AI coding assistant.

Simply type:

```
/packmind-update-playbook create a command to add a new endpoint in the API.
```
...
This command is automatically available when you run `packmind skills init` or distribute packages to your repository.
...
You can also use the `/packmind-update-playbook` skill to have your agents update an existing command:

```
/packmind-update-playbook Update the create-api-endpoint command to use the newly defined guard system.
```

Alternatively, you can manually edit the files corresponding to the command and use the `playbook` command of the cli to submit them:

```
packmind playbook add .claude/commands/create-api-endpoint.md
...
The **slug** is automatically generated from the name and used throughout the system. If you create multiple commands with similar names in the same [space](/concepts/spaces), Packmind appends numbers to keep slugs unique (e.g., `"add-api-endpoint"`, `"add-api-endpoint-1"`).
