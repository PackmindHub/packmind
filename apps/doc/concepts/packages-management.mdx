...
You can add items to packages automatically when creating commands or standards via the MCP server by including the `packageSlugs` parameter. Skills are added to packages through the web interface after uploading them via CLI.
