...
- **Linux x64**: `packmind-linux-x64-{version}`
- **Linux arm64**: `packmind-linux-arm64-{version}`
- **macOS arm64**: `packmind-macos-arm64-{version}` (signed and notarized)
- **Windows x64**: `packmind-windows-x64-{version}.exe`

**For Linux/macOS**, make the executable runnable:

```bash
chmod +x packmind-*-{version}
```
...
```bash
# Linux/macOS
sudo mv packmind-*-{version} /usr/local/bin/packmind
