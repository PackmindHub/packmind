...
<Note>
  **The installed command is `packmind`** — Earlier versions installed it as
  `packmind-cli`. That name still works for now, but it is deprecated and will
  be removed in a future release, so point any scripts or CI jobs at `packmind`.
</Note>
...
Once you're authenticated, the first command to run inside your project is `packmind init`. This sets up Packmind for the repository and must be run before any onboarding or artifact generation.

From the root of your project (which must be a Git repository), run:
...
- Prompt you to select which AI coding agents you use (Claude Code, Cursor, GitHub Copilot, etc.)
- Save your selection to a `packmind.json` file at the project root
- Install Packmind's default skills (including `packmind-onboard` and `packmind-update-playbook`) into the appropriate directories for your selected agents

See the [Init Command](/tools/cli#init-command) reference for the full output and details.

## Next Steps

Now that you have the CLI installed, authenticated, and your project initialized, you can create your first standards and commands:
