...
```
/packmind-update-playbook create a Packmind command to add a new endpoint to our API
```
...
<Tip>
  **Guided Creation** — The `/packmind-update-playbook` skill provides
...
- **Use the current conversation**
  ...
  **Example**
  ...
  ```text
  /packmind-update-playbook Create a Packmind command for adding a new endpoint based on your last suggested changes.
  ```
...
- **Mention example files or directories**
  ...
  **Example**
  ...
  ```text
  /packmind-update-playbook Using the file `MyPageFile.tsx`, create a Packmind command for bootstrapping a new page in the frontend.
  ```
...
- **Use a Git commit diff**
  ...
  **Example**
  ...
  ```text
  /packmind-update-playbook Read commit abcd123 and create a command for migrating an Angular component to React.
  ```
