...
- You must be authenticated: run `packmind login`
...
```bash
packmind playbook add .claude/skills/*/
packmind playbook submit --no-review
```
...
The same applies when submitting. If a skill's name is already taken, it is skipped and stays staged while the rest are submitted — rename it and run `packmind playbook submit --no-review` again to finish the job.
