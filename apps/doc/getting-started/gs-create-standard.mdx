...
```
/packmind-update-playbook create a Packmind standard about error handling
```
...
When using a coding agent like Claude Code, you can invoke the `/packmind-update-playbook` skill to get guided assistance through the entire standard creation process.

### Using the skill

1. Open your coding agent (e.g., Claude Code)
2. Type `/packmind-update-playbook` followed by a description of the standard you want to create
3. The agent will analyze your codebase and iterate with you on the draft
...
3. **Review** — Showing you the complete standard for approval
4. **Submission** — Creating the standard via the CLI
...
You can also create standards directly using the Packmind CLI with a markdown file.

```bash
packmind playbook add .packmind/standards/my-standard.md
```
...
The optional `## Scope` section holds the glob patterns the standard applies to,
so your coding assistants only load it for matching files. Write globs rather
than prose, separated by commas on one line or one bullet per pattern. Leave the
section out for a standard that applies everywhere.

<Info>
  The standard markdown should be located in a directory where instructions are
  available.

    Such directories include:
    <ul>
        <li>`.packmind/standards`</li>
        <li>`.claude/rules`</li>
        <li>`.cursor/rules`</li>
