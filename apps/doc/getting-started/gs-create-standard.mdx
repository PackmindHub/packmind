...
# From stdin
echo '{"name":"Error Handling","description":"...","scope":"All services","rules":[{"content":"Validate inputs"}]}' | packmind-cli standards create
```
...
<Tip>
  ...
  repository or want to automate standard creation. Stdin support makes it easy
  to pipe JSON from scripts or AI agents. See [CLI: Standards
  Command](/tools/cli#standards-command) for more details.
</Tip>
