...
- `SMTP_TLS_REJECT_UNAUTHORIZED` - Whether the SMTP server's TLS certificate must be valid and trusted. Defaults to `true`; set it to `false` only if your SMTP server uses a certificate Packmind cannot verify

<Warning>
  Setting `SMTP_TLS_REJECT_UNAUTHORIZED` to `false` disables certificate
  verification for outgoing mail, which exposes your messages to
  man-in-the-middle interception. If your SMTP server uses a self-signed
  certificate or a private Certificate Authority, prefer trusting that CA with
  `NODE_EXTRA_CA_CERTS` (see [Add custom CA
  certificates](#add-custom-ca-certificates-docker-compose)) instead.
</Warning>
