...
3. Edit `docker-compose.yml` and uncomment the certificate lines in the `api` service:
...
You can override the `api` section of the `docker-compose.yaml` file.
...
- `SMTP_TLS_REJECT_UNAUTHORIZED` - Whether the SMTP server's TLS certificate must be valid and trusted. Defaults to `false`; set it to `true` to enable certificate verification. Only `true` and `false` are accepted — any other value is rejected with an error rather than treated as `false`, so a typo cannot silently leave verification off

<Warning>
  By default Packmind does **not** verify the TLS certificate of your SMTP
  server, which leaves outgoing mail open to man-in-the-middle interception. Set
  `SMTP_TLS_REJECT_UNAUTHORIZED` to `true` to turn verification on. If your SMTP
  server uses a self-signed certificate or a private Certificate Authority,
  trust that CA with `NODE_EXTRA_CA_CERTS` (see [Add custom CA
  certificates](#add-custom-ca-certificates-docker-compose)) so you can enable
  verification rather than leaving it off.
</Warning>
