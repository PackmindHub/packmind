...
<Note>
  **Default skills included** — All distributions automatically include
  ...
  skills with AI assistance). These are added alongside your package contents
  regardless of which distribution method you use.
</Note>
...
2. **Using packmind** - Install packages directly from your terminal, useful for CI/CD pipelines and local development
...
- **Monorepos** - Use `packmind install --recursive` to install packages for all `packmind.json` files in the repository
...
```bash
packmind remove <package-slug>
```

For example:

```bash
packmind remove backend-standards
```
