...
When your organization tracks a repository and branch (see the [Track Command](/tools/cli#track-command)), Packmind shows the **branch you currently track**. Distributions made on other branches of the same repository are hidden, so the view stays focused on the branch your team works from today.
...
<Note>
  **Only one branch at a time** — Packmind shows the tracked branch's history
  ...
  ask an organization admin to move tracking to it with `packmind-cli track
  --update`.
</Note>
...
`packmind-cli track --remove` from a clone of it (see the [Track Command](/tools/cli#track-command)).
The repository disappears from Distribution History, the Deployments overview and the deployment
targets.
