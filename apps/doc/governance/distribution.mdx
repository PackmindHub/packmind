...
2. **Using the Packmind CLI** - Install packages directly from your terminal, useful for CI/CD pipelines and local development
...
## Distribute via the Packmind CLI

You can also distribute packages directly from your terminal using the `packmind` command:
...
```bash
packmind install backend-standards frontend-react
...
- **`packmind git info`** — reports that the branch is not in the repository and offers to move tracking.
...
<Note>
  **Needs a Git connection** — checking a branch means asking your Git provider,
  ...
  connection. A repository Packmind knows only from `packmind install` carries
  no credentials to ask with, and `packmind git info` is the signal there.
</Note>
...
<Note>
  **Only one branch at a time** — Packmind shows the tracked branch's history
  ...
  ask an organization admin to move tracking to it with `packmind git track
  --update`.
</Note>
...
`packmind git untrack` from a clone of it (see [Git Untrack](/tools/cli#git-untrack)).
...
While a repository is untracked, `packmind install` behaves exactly as it does for a repository
...
```bash
packmind uninstall <package-slug>
```

For example:

```bash
packmind uninstall backend-standards
```
