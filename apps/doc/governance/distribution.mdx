...
When your organization tracks a repository and branch (see the [Track Command](/tools/cli#track-command)), that history shows the **branch you currently track**. Distributions made on other branches of the same repository are hidden, so the view stays focused on the branch your team works from today.

Repositories you have never tracked are unaffected — their history keeps showing every branch.
