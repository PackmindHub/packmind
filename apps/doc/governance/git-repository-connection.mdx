...
<Tip>
  **Prefer the terminal?** — You can list and add token-authenticated
  connections and manage repositories from the CLI with the `git` command. See
  [Git Command](/tools/cli#git-command).
</Tip>
