---
title: 'Linter: Automated Detection of Coding Rules'
---
...
<Warning>
  **LLM Connection Required** — The linter feature requires an LLM (Large
  Language Model) connection to generate detection programs. This is **enabled
  by default** on Packmind Cloud. For self-hosted installations, you need to
  configure your OpenAI API key. See [Install Packmind on your
  server](/getting-started/gs-install-self-hosted#connect-your-llm-optional-but-important)
...
To add code examples to a rule, see the [Standards Management](/concepts/standards-management) documentation.
...
<Tip>
  **Automatic Assessment** — Assessment is triggered automatically whenever you
  update code examples or Detection Clues. There is no manual trigger for
  assessment - the system re-evaluates detectability each time relevant inputs
  change (and no program generated successfully)
</Tip>
...
You can test the program behavior against source code using the sandbox module in the interface, or using the [CLI](/tools/cli#lint-command).
...
Each active detection program has a **severity** level that determines how violations are treated:

### Error (default)

Rules with `error` severity are **blocking**. When the linter detects a violation from an error-level rule, it exits with a non-zero exit code. This makes errors ideal for **CI/CD gates** — your pipeline will fail if any error-level violations are found.

### Warning

Rules with `warning` severity are **informational**. Violations are reported in the output, but they do **not** cause a non-zero exit code. Warnings won't block your pipeline — they serve as guidance for developers without enforcing a hard stop.

### Configuring Severity

Severity can be configured per rule and per language from the rule's detection settings in the Packmind web interface. By default, all rules are set to `error` severity.

<Tip>
  **When to use warnings** — Warnings are useful for rules you want to gradually
  roll out, stylistic preferences, or best practices that shouldn't block
  deployments. Start with `warning` severity and promote to `error` once the
  team is aligned.
</Tip>
...
The CLI supports testing both through local `packmind.json` configuration files and through distributed standards. For complete CLI testing instructions, see the [CLI documentation](/tools/cli).
...
You can exclude specific files or directories from lint analysis by creating a `.packmindignore` file. Any file matching a pattern in `.packmindignore` will be skipped entirely during linting.

### File Location

Place a `.packmindignore` file at the root of your project. In a **Git repository**, the CLI collects patterns from all `.packmindignore` files found between the linted directory and the Git repository root, merging them together. Outside a Git repository, only the `.packmindignore` in the linted directory is used.
...
- Simple names (e.g., `generated`) match any directory or file with that name at any level
- Use `*` to match anything except path separators
- Use `**` to match anything including subdirectories
...
# Ignore legacy JavaScript files in src
src/*.legacy.js
```

<Note>
  The following paths are **always excluded** by default, even without a
  `.packmindignore` file: `node_modules`, `dist`, `.git`, `*.min.*` files, and
  `*.map` files.
</Note>

## Rule Severity

Each active detection program can be assigned a severity level that controls how violations are reported when running the linter.

### Severity Levels

**ERROR** — A blocking severity. When the linter detects a violation for a rule with ERROR severity, it exits with a non-zero exit code. This is suitable for rules that must never be violated and should block CI/CD pipelines.

**WARNING** — An informational severity. Violations are reported in the linter output but the exit code remains zero. This is suitable for rules that are important to highlight but should not block the build.

### Configuring Severity

You can configure the severity of each rule's detection program directly from the rule's detection settings in the Packmind interface. The severity selector appears next to the active detection program for each language.

- Navigate to your rule's detection settings
- Locate the active detection program for the desired language
- Use the severity dropdown to switch between **ERROR** and **WARNING**

Severity can also be set programmatically via the Packmind API or configured in your `packmind.json` file when using the CLI.

## Related Documentation

- [Standards Management](/concepts/standards-management): Learn how to create rules and add code examples
- [CLI](/tools/cli): Test detection programs from the command line
