---
title: 'Use Packmind Cloud'
---

## Overview[​](#overview 'Direct link to Overview')

Packmind Cloud is a fully managed service that allows you to use Packmind without managing any infrastructure. Simply sign up and start using the platform immediately.

## Availability[​](#availability 'Direct link to Availability')

Packmind Cloud is currently available at [https://app.packmind.ai](https://app.packmind.ai) (EU region).

**US region will come soon.**

## Security & Privacy[​](#security--privacy 'Direct link to Security & Privacy')

For comprehensive information about data handling, security measures, AI integration, and compliance, please see our [Security & Privacy documentation](/security-privacy).

[Edit this page](https://github.com/PackmindHub/packmind/apps/doc/docs/gs-install-cloud.md)
