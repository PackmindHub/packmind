...
<Note>
  Change Proposals require **CLI version 0.21.0 or later**. Run `packmind
...
2. **Edit local files manually and use the CLI** — modify distributed artifact files directly, then use `packmind diff` commands to preview and submit your changes.
...
```bash
packmind diff --submit -m "Clarify error handling rules for API endpoints"
```
...
```bash
packmind diff --include-submitted
```
...
```bash
packmind diff add <path> -m "Description of the new artifact"
```
...
```bash
packmind diff remove <path> -m "Reason for removing the artifact"
```
...
Always submit your changes before running `packmind install` to avoid losing your work.
