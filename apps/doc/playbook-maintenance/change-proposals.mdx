...
<Note>
  Change Proposals require **CLI version 0.23.0 or later**. Run `packmind
  --version` to check your version — on 0.23.x the command is `packmind-cli
  --version`, which is the only name that install has — and see [CLI
...
2. **Edit local files manually and use the CLI** — modify distributed artifact files directly, then use `packmind playbook` commands to stage and submit your changes.
...
2. Run `packmind playbook diff` to preview what changed
3. Run `packmind playbook status` to see staged and untracked changes
4. Run `packmind playbook add <path>` to stage the changes you want to submit
5. Run `packmind playbook submit -m "message"` to submit staged changes as proposals
...
```bash
packmind playbook diff --include-submitted
```

To limit the analysis to a specific path:

```bash
packmind playbook diff -p .claude/rules/
```
...
| **Standards** | `.claude/rules/`, `.cursor/rules/`, `.github/instructions/`, `.continue/rules/`, `.kiro/steering/`, `.packmind/standards/`              |
| **Skills**    | `.claude/skills/`, `.cursor/skills/`, `.github/skills/`, `.gitlab/duo/skills/`, `.opencode/skills/`, `.agents/skills/`, `.kiro/skills/` |
...
```bash
packmind playbook submit -m "Clarify error handling rules for API endpoints"
```
...
To resolve this, either rename your local artifact file or run `packmind playbook unstage <path>` to remove it from the staged changes.

<Tip>
  If you see an error like **Failed to submit "{name}": {artifactType} is
  outdated, please run packmind install to update it**, it means the local
  artifact version is stale. Run `packmind install` to fetch the latest
...
<Warning>
  While changes are pending (not yet accepted), running `packmind install` will
  overwrite your local modifications with the server version. This will erase
  any unstaged or unsubmitted changes.
</Warning>

Always submit your staged changes with `packmind playbook submit` before running `packmind install` to avoid losing your work.
