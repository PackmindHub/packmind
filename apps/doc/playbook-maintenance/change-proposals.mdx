...
2. **Edit local files manually and use the CLI** — modify distributed artifact files directly, then use `packmind-cli playbook` commands to stage and submit changes as a batch, or `packmind-cli diff` commands to preview and submit individual changes.
...
There are two CLI workflows for submitting changes:

1. **Playbook staging workflow (recommended)** — stage multiple changes, review them, and submit as a batch
2. **Direct diff submission** — preview and submit individual changes immediately

### Staging and submitting with `playbook`

The `playbook` commands let you stage multiple changes locally, review what will be submitted, and then submit everything as a single batch of proposals.

#### Stage a change

Use `playbook add` to stage a local modification, addition, or update for submission:

```bash
packmind-cli playbook add .claude/rules/naming-conventions.md
```

For skills, point to the skill directory:

```bash
packmind-cli playbook add .claude/skills/my-skill/
```

If your organization has multiple spaces, use the `--space` flag to target a specific space:

```bash
packmind-cli playbook add .claude/rules/naming-conventions.md --space "my-space"
```

Sample output:

```
Staged "Naming Conventions" (standard, updated) to playbook in space "my-space"
```

#### Stage a removal

Use `playbook rm` to stage the removal of an artifact:

```bash
packmind-cli playbook rm .claude/commands/old-workflow.md
```

#### Review staged changes

Use `playbook status` to see what is currently staged and what local changes are untracked:
...
```
Staged changes:
  updated  .claude/rules/naming-conventions.md (standard)
  removed  .claude/commands/old-workflow.md (command)

Untracked changes:
  modified .claude/rules/error-handling.md (standard)
```

#### Unstage a change

Use `playbook unstage` to remove a change from staging without discarding the local edit:

```bash
packmind-cli playbook unstage .claude/rules/naming-conventions.md
```

#### Submit staged changes

Use `playbook submit` to submit all staged changes as change proposals:

```bash
packmind-cli playbook submit -m "Updated naming conventions for API endpoints"
```

If you omit the `-m` flag in interactive mode, an editor will open for you to write the message.

### Direct submission with `diff`

For submitting individual changes without staging, use the `diff` commands:
