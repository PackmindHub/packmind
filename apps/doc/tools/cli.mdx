...
- **`git connection list`** - List Git connections (providers) in your organization
- **`git connection add`** - Add a Git connection with an access token
- **`git repo list`** - List repositories managed under a connection (or those available to manage)
- **`git repo add`** - Start managing a repository under a connection
...
Set which repository and branch Packmind tracks for your project. The tracked branch is the anchor Packmind uses to attribute governance and adoption activity, so declaring it tells Packmind where to follow your team's work. Only installs that run on the tracked repository and branch are recorded — see [Distribution Recording](#distribution-recording-tracked-repository-and-branch).
...
Manage the Git connections and repositories your organization uses to distribute standards, commands, and skills — directly from the terminal. This mirrors what you can do from **Settings** → **Git Providers** in the web app. For the full picture of how connections work, see [Git Repository Connection](/governance/git-repository-connection).

The `git` command has two subgroups:

- **`git connection`** — manage Git connections (also called providers), such as a GitHub or GitLab connection.
- **`git repo`** — manage the repositories handled under a connection.

<Info>
  **Administrator only** — Adding a connection or a repository requires an
  organization admin. Listing is available to any member. If you are not an
  admin, `git connection add` and `git repo add` report that you need
  administrator permissions and make no change.
</Info>

<Info>
  **GitHub App connections** — The CLI creates **token-authenticated**
  connections. To connect GitHub with the Packmind GitHub App (the recommended
  option for GitHub), use the web app — see [Connecting
  GitHub](/governance/git-repository-connection#connecting-github).
</Info>

### List Connections

List the Git connections available in your organization:

```bash
packmind-cli git connection list
```

For each connection, the output shows its ID, display name, type, URL, and whether authentication is configured. You need the **connection ID** to manage repositories under it.
...
- My GitLab
    ID:   3f8c1a90-1c2b-4e77-9a0e-2f5b7c9d1e34
    Type: gitlab
...
Add a new Git connection authenticated with an access token:

```bash
packmind-cli git connection add <token> --displayName "My GitLab" --type gitlab --url https://gitlab.example.com
```

**Arguments:**

- `<token>` — Access token used to authenticate against the provider

**Options:**

| Flag            | Description                                                                         |
| --------------- | ----------------------------------------------------------------------------------- |
| `--displayName` | Human-readable name to identify the connection later                                |
| `--type`        | Provider type: `github` or `gitlab`                                                 |
| `--url`         | Base URL of the provider (required for self-hosted instances, e.g. a GitLab server) |

**Example output:**

```
Git connection created: My GitLab (3f8c1a90-1c2b-4e77-9a0e-2f5b7c9d1e34)
```

### List Repositories

List the repositories currently managed under a connection:

```bash
packmind-cli git repo list --connectionId <connection-id>
```

To discover repositories that Packmind **can** manage but that aren't managed yet, add `--show-available`:

```bash
packmind-cli git repo list --connectionId <connection-id> --show-available
```

Available repositories are paginated. Use `--page` to fetch a specific page:

```bash
packmind-cli git repo list --connectionId <connection-id> --show-available --page 2
```

### Add a Repository

Start managing a repository under a connection, using the `owner/repo` form:

```bash
packmind-cli git repo add <owner>/<repo> --connectionId <connection-id>
```

By default, Packmind uses the repository's **default branch**. To manage a different branch, pass `--branch`:

```bash
packmind-cli git repo add myOrga/myRepo --connectionId <connection-id> --branch develop
```

**Example output:**

```
Repository added: myOrga/myRepo (main)
```
...
- **Problem**: `You need to be an organization administrator to add a git connection.` (or `to manage a repository.`)
- **Solution**: Adding connections and repositories is restricted to organization admins. Ask an admin to run the command, or to grant you the admin role.

- **Problem**: `Could not determine the default branch for <owner>/<repo>.`
- **Solution**: The repository was not found among the connection's available repositories. Re-run `git repo add` with an explicit `--branch`, or confirm the connection can access that repository with `git repo list --connectionId <id> --show-available`.

- **Problem**: Adding a repository reports it already exists.
- **Solution**: The repository and branch are already managed under this organization. Use `git repo list --connectionId <id>` to review what is already managed.
...
When you install packages inside a git repository, Packmind records the deployment as a **distribution** so it shows up in your governance and adoption views. To keep those views meaningful, a distribution is recorded **only when you install on the repository and branch your organization tracks** (see the [Track Command](#track-command)).
...
- **The repository is not tracked** — no repository is tracked for this project yet:
  ...
  ```
  Distribution not recorded — my-org/my-repo is not tracked in Packmind. Ask an admin to run `packmind track` to start tracking it.
  ```
...
<Info>
  **Anyone can install** — Recording a distribution is not restricted to admins.
  ...
  admin (see the [Track Command](#track-command)).
</Info>
...
<Note>
  **Not seeing your activity in Packmind?** — Confirm you are on the tracked
  ...
  branch with `packmind-cli track`. If the branch was tracked before and is not
...
- [Git Repository Connection](/governance/git-repository-connection): Connect GitHub and GitLab providers and manage repositories
