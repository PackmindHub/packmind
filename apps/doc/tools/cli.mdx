...
<Note>
  **Renamed** — These used to be the top-level `packmind-cli track` and
  `packmind-cli untrack` commands. The old names now exit with an error naming
  their replacement.
</Note>
...
```
Repository my-org/my-repo is already tracked on branch main. Run `packmind-cli git track --update` to move it to dev.
```
...
```bash
packmind-cli git track --branch main
```
...
```
No branch is checked out for my-org/my-repo — HEAD is detached. Check a branch out, or name one with `packmind-cli git track --branch <name>`.
```
...
```
Nothing is tracked yet — run `packmind-cli init` or `packmind-cli git track` to start tracking.
```
...
<Tip>
  **During onboarding** — `packmind-cli init` also offers to track the current
  ...
  setup. Use `packmind-cli git track` later to set or move it.
</Tip>
...
The repository disappears from Distribution History and the Deployments overview. **Nothing is deleted** — run `packmind-cli git track` on the same branch again and the whole history comes back, with new distributions appended. See [Removing Tracking](/governance/distribution#removing-tracking).

While the repository is untracked, `packmind-cli install` behaves as it does for a repository that was never tracked: it completes locally and tells you the distribution was not recorded.
...
```
Packmind tracks my-org/my-repo on branch 'main'.
You are on 'dev', so distributions from here are not recorded. Run `packmind-cli git track --update` to move tracking to 'dev'.
```

When the repository is not tracked at all:

```
my-org/my-repo is not tracked in Packmind. Run `packmind-cli git track` to track branch 'main'.
```
...
- **Solution**: Use `packmind-cli git track` (or `packmind-cli git untrack` in place of `packmind-cli track --remove`).
...
- **The repository is not tracked** — no repository is tracked for this project yet:
  ...
  ```
  Distribution not recorded — my-org/my-repo is not tracked in Packmind. Ask an admin to run `packmind-cli git track` to start tracking it.
  ```
...
<Note>
  **Not seeing your activity in Packmind?** — Confirm you are on the tracked
  ...
  branch with `packmind-cli git track`. If the branch was tracked before and is
  not anymore, its distributions are still kept — they reappear when tracking
  moves back. See [Distribution History and the Tracked
