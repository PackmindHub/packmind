...
Use the `--remove` flag to take the repository out of Packmind's governance views. Packmind asks you to confirm before applying it:

```bash
packmind-cli track --remove
```

```
Remove Packmind's tracking of my-org/my-repo on branch main? Distributions already recorded are kept and reappear if you track it again. (Y/n)
...
The repository disappears from Distribution History, the Deployments overview and the deployment targets. **Nothing is deleted** — run `packmind-cli track` on the same branch again and the whole history comes back, with new distributions appended. See [Removing Tracking](/governance/distribution#removing-tracking).
...
Removing tracking on a repository that is not tracked changes nothing and reports it:

```
Repository is not tracked in 'my-organization' organization
```

`--update` and `--remove` cannot be combined — pick one.
