...
- **`standard create`** - Create a coding standard from a playbook file
...
- **packmind-standard-create** — A skill for creating coding standards

After running this command, you can use the `/packmind-create-skill` command in your AI coding assistant to create new skills interactively.
...
<Tip>
  **Automatic installation** — Default skills are also installed automatically
  ...
  distributed to your repository via the web app.
</Tip>
...
The playbook file is a JSON file with the following structure:
...
| ---------- | -------- | --------------------------------------------------- |
| `content`  | Yes      | Rule description (should start with an action verb) |
