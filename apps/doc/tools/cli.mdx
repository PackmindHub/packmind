...
Manage AI coding agents configured in your `packmind.json` files. Unlike `config agents` which uses an interactive prompt, the `agents` command provides programmatic subcommands for listing, adding, and removing agents — useful for scripting and CI workflows.
...
```bash
packmind-cli agents list --path ./apps/api
```
...
```bash
packmind-cli agents add claude cursor
```

Use `--path` to target a specific directory:

```bash
packmind-cli agents add --path ./packages/core claude
```
...
```bash
packmind-cli agents rm cursor copilot
```

Use `--path` to target a specific directory:

```bash
packmind-cli agents rm --path ./apps/frontend claude
```
...
```bash
echo '{"name":"Error Handling","description":"...","scope":"All services","rules":[{"content":"Validate inputs at boundaries"}]}' | packmind-cli standards create
```

Using a heredoc:

```bash
packmind-cli standards create <<'EOF'
{
  "name": "Error Handling",
  "description": "Ensure errors are handled predictably.",
  "scope": "All services",
...
```bash
cat my-command.json | packmind-cli commands create
```

Using a heredoc:

```bash
packmind-cli commands create <<'EOF'
{
  "name": "Add a New REST Endpoint",
