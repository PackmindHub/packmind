...
- **`track`** - Set the repository and branch Packmind tracks for your project (`--branch` to name it explicitly)
- **`untrack`** - Remove Packmind's tracking of the current repository
...
<Info>
  **Organization admins only** — Only organization admins can set, move or
  ...
  you cannot change the tracked branch.
</Info>
...
```bash
packmind-cli track --branch main
```
...
Take a repository out of Packmind's governance views. Tracking applies to the repository, so `untrack` takes no branch — it removes whichever branch is currently tracked.
...
```bash
packmind-cli untrack
```

```
Stop tracking my-org/my-repo? Packmind currently tracks branch main. Distributions already recorded are kept and reappear if you track it again. (y/N)
...
The repository disappears from Distribution History and the Deployments overview. **Nothing is deleted** — run `packmind-cli track` on the same branch again and the whole history comes back, with new distributions appended. See [Removing Tracking](/governance/distribution#removing-tracking).
...
Running `untrack` on a repository that is not tracked changes nothing and reports it, exiting successfully so the command is safe to repeat:
