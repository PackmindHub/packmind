...
- **`packages move`** - Move standards, commands, or skills into a package, removing them from every other one
- **`packages remove`** - Remove standards, commands, or skills from a package
...
- **Kiro** - Artifacts in `.kiro/` directory
...
<Note>
  **One item, one package** — An item belongs to a single package. If the item
  already belongs to another one, `packages add` changes nothing and prints the
  `packages move` command that would relocate it:

```
$ packmind packages add --to backend --command create-api-endpoint

Command "Create API endpoint" already belongs to package "@my-space/api"

Run `packmind packages move --command create-api-endpoint --to @my-space/backend` to move the command.
```

</Note>

### Move Items Between Packages

```bash
packmind packages move --to <package-slug> --standard <standard-slug>
packmind packages move --to <package-slug> --command <command-slug>
packmind packages move --to <package-slug> --skill <skill-slug>
```

Adds the item to the target package and removes it from **every** other package it belonged to. An item already in the target package and nowhere else is left untouched.
...
| ------------ | --------------------------------------------- |
| `--to`       | Target package slug (required)                |
| `--standard` | Slug of the standard to move into the package |
| `--command`  | Slug of the command to move into the package  |
| `--skill`    | Slug of the skill to move into the package    |

**Example:**

```
$ packmind packages move --command create-api-endpoint --to @my-space/backend

Command "Create API endpoint" has been added to package "@my-space/backend"
It has been removed from "@my-space/api"
```

When the item came from several packages, they are listed:
...
```bash
packmind packages remove --from <package-slug> --standard <standard-slug>
packmind packages remove --from <package-slug> --command <command-slug>
packmind packages remove --from <package-slug> --skill <skill-slug>
```
...
| Flag         | Description                                     |
| ------------ | ----------------------------------------------- |
| `--from`     | Slug of the package to remove from (required)   |
| `--standard` | Slug of the standard to remove from the package |
| `--command`  | Slug of the command to remove from the package  |
| `--skill`    | Slug of the skill to remove from the package    |

**Example:**

```
$ packmind packages remove --command create-api-endpoint --from @my-space/backend

Command "Create API endpoint" has been removed from package "@my-space/backend"
It does not belong to any package anymore
```

<Warning>
  An item that belongs to no package is no longer installed or distributed
  anywhere. The command always states what the item still belongs to, so you can
