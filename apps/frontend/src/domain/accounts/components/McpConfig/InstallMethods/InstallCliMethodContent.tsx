...
export const InstallCliMethodContent: React.FC<IMethodContentProps> = ({
  method,
  token,
  url,
  cliLoginCode,
  ...
  const command = method.getCliCommand?.(token, url, cliLoginCode);
  ...
  return (
    <PMVStack gap={4} width="100%" alignItems="flex-start">
      ...
      <CopiableTextarea
        ...
        rows={2}
        onCopy={() =>
          analytics.track('mcp_installed', { method: 'install-cli', agent })
        }
      />
      {onCantUseMcp && (
