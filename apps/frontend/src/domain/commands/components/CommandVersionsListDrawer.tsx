...
import { useGetCommandVersionsQuery } from '../api/queries/CommandsQueries';
import { CommandId, WithTimestamps } from '@packmind/types';

interface CommandVersionsListDrawerProps {
  recipeId: CommandId;
}

export const CommandVersionsListDrawer: React.FC<
  CommandVersionsListDrawerProps
> = ({ recipeId }) => {
  const { data: versions, isLoading } = useGetCommandVersionsQuery(recipeId);
