...
const CONVENTIONAL_COMMIT_EXAMPLE: CommandExample = {
...
const GENERATE_PLAYWRIGHT_TEST_EXAMPLE: CommandExample = {
...
interface CommandsExampleDialogButtonProps {
  example: CommandExample;
}

const CommandsExampleDialogButton = ({
  example,
}: CommandsExampleDialogButtonProps) => {
  return (
    <PMDialog.Root
      size="xl"
      placement="center"
...
export const CommandsExampleDialog = () => {
  return (
    <PMHStack mt={4} gap={2}>
      {RECIPE_EXAMPLES.map((example) => (
        <CommandsExampleDialogButton key={example.title} example={example} />
      ))}
    </PMHStack>
  );
};
