...
import { CommandCentricView } from './CommandCentricView';
import {
  createCommandDeploymentStatus,
...
describe('RecipeCentricView', () => {
  it('displays recipe name', () => {
    const recipes = [
      createCommandDeploymentStatus({
        recipe: commandFactory({ name: 'Test Recipe' }),
      }),
    ];

    renderWithProvider(<CommandCentricView recipes={recipes} />);
  ...
  describe('when recipe is undeployed', () => {
    beforeEach(() => {
      const recipes = [
        createCommandDeploymentStatus({
          recipe: commandFactory({ name: 'Undeployed Recipe' }),
          deployments: [],
  ...
  describe('filtering', () => {
    describe('when filtering by search term', () => {
      beforeEach(() => {
        const recipes = [
          ...
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Other Recipe' }),
          }),
        ];

        renderWithProvider(
          <CommandCentricView recipes={recipes} searchTerm="test" />,
        );
      });
    ...
    describe('when filtering by outdated status', () => {
      beforeEach(() => {
        const recipes = [
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Outdated Recipe' }),
            hasOutdatedDeployments: true,
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Up-to-date Recipe' }),
            hasOutdatedDeployments: false,
          }),
        ];

        renderWithProvider(
          <CommandCentricView recipes={recipes} showOnlyOutdated={true} />,
        );
      });
    ...
    describe('when filtering by both search and outdated status', () => {
      beforeEach(() => {
        const recipes = [
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Test Outdated Recipe' }),
            hasOutdatedDeployments: true,
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Test Up-to-date Recipe' }),
            hasOutdatedDeployments: false,
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Other Outdated Recipe' }),
            hasOutdatedDeployments: true,
          }),
        ];
    ...
    describe('when filtering by undeployed status', () => {
      beforeEach(() => {
        const recipes = [
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Distributed Recipe' }),
            deployments: [createRepositoryDeploymentInfo()],
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Undeployed Recipe 1' }),
            deployments: [],
            hasOutdatedDeployments: false,
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Undeployed Recipe 2' }),
            deployments: [],
            hasOutdatedDeployments: false,
          }),
        ];
        ...
        renderWithProvider(
          <CommandCentricView recipes={recipes} showOnlyUndeployed={true} />,
        );
      });
    ...
    describe('when filtering by both search and undeployed status', () => {
      beforeEach(() => {
        const recipes = [
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Test Distributed Recipe' }),
            deployments: [createRepositoryDeploymentInfo()],
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Test Undeployed Recipe' }),
            deployments: [],
            hasOutdatedDeployments: false,
          }),
          createCommandDeploymentStatus({
            recipe: commandFactory({ name: 'Other Undeployed Recipe' }),
            deployments: [],
            hasOutdatedDeployments: false,
          }),
        ];
  ...
  describe('empty states', () => {
    describe('when no recipes match search term', () => {
      beforeEach(() => {
        ...
        renderWithProvider(
          <CommandCentricView recipes={recipes} searchTerm="nomatch" />,
        );
      });
    ...
    describe('when no recipes exist', () => {
      beforeEach(() => {
        renderWithProvider(<CommandCentricView recipes={[]} />);
      });
