...
jest.mock('../api/queries/SkillsQueries', () => ({
  ...
  useDeleteSkillsBatchMutation: jest.fn(),
}));
...
jest.mock('./SkillsBlankState', () => ({
  SkillsBlankState: () => null,
}));
...
describe('SkillsList', () => {
  beforeEach(() => {
    (useGetSkillsQuery as jest.Mock).mockReturnValue({
      data: mockSkills,
      isLoading: false,
      isError: false,
    });
    (useDeleteSkillsBatchMutation as jest.Mock).mockReturnValue({
      mutateAsync: jest.fn(),
      isPending: false,
    });
  ...
  describe('select all with active search filter', () => {
    it('only selects filtered skills when clicking the header checkbox', async () => {
      renderWithProviders(<SkillsList orgSlug="my-org" />);
    ...
    it('does not select skills that were filtered out', async () => {
      renderWithProviders(<SkillsList orgSlug="my-org" />);
    ...
    it('selects all skills when no filter is applied', async () => {
      renderWithProviders(<SkillsList orgSlug="my-org" />);
