...
import { registerAddRuleToStandardWorkflowTool } from './addRuleToStandardWorkflow.tool';
...
describe('addRuleToStandardWorkflow.tool', () => {
  ...
  describe('registerAddRuleToStandardWorkflowTool', () => {
    ...
    beforeEach(() => {
      (mcpServer.tool as jest.Mock).mockImplementation(
        (name, description, schema, handler) => {
          if (name === 'packmind_add_rule_to_standard_workflow') {
            toolHandler = handler;
          }
        },
      );
    ...
    describe('when userContext is available', () => {
      it('tracks analytics event with correct step', async () => {
        ...
        expect(mockAnalyticsAdapter.trackEvent).toHaveBeenCalledWith(
          ...
          'org-123',
          'mcp_tool_call',
          { tool: 'packmind_add_rule_to_standard_workflow', step: 'drafting' },
        );
      });
    });
    ...
    it('tracks analytics with different steps for each valid step', async () => {
      ...
      for (const step of steps) {
        ...
        expect(mockAnalyticsAdapter.trackEvent).toHaveBeenCalledWith(
          ...
          'org-123',
          'mcp_tool_call',
          { tool: 'packmind_add_rule_to_standard_workflow', step },
        );
      }
    });
