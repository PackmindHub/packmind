...
import { registerCreateRecipeWorkflowTool } from './createRecipeWorkflow.tool';
import { ToolDependencies, UserContext } from './types';

describe('createRecipeWorkflow.tool', () => {
  ...
  describe('registerCreateRecipeWorkflowTool', () => {
    ...
    beforeEach(() => {
      (mcpServer.tool as jest.Mock).mockImplementation(
        (name, description, schema, handler) => {
          if (name === 'packmind_create_recipe_workflow') {
            toolHandler = handler;
          }
        },
      );
    ...
    describe('when userContext is available', () => {
      it('tracks analytics event with correct step', async () => {
        ...
        expect(mockAnalyticsAdapter.trackEvent).toHaveBeenCalledWith(
          ...
          'org-123',
          'mcp_tool_call',
          { tool: 'packmind_create_recipe_workflow', step: 'drafting' },
        );
      });
    });
    ...
    describe('when no step provided', () => {
      it("defaults to 'initial-request'", async () => {
        ...
        expect(mockAnalyticsAdapter.trackEvent).toHaveBeenCalledWith(
          ...
          'org-123',
          'mcp_tool_call',
          { tool: 'packmind_create_recipe_workflow', step: 'initial-request' },
        );
      });
    });
    ...
    it('tracks analytics with different steps for each valid step', async () => {
      ...
      for (const step of steps) {
        ...
        expect(mockAnalyticsAdapter.trackEvent).toHaveBeenCalledWith(
          ...
          'org-123',
          'mcp_tool_call',
          { tool: 'packmind_create_recipe_workflow', step },
        );
      }
    });
