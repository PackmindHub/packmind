...
export class ClaudeDeployer implements ICodingAgentDeployer {
  private static readonly CLAUDE_PATH = 'CLAUDE.md';
  ...
  async deployRecipes(
    ...
    if (updatedContent !== existingContent) {
      fileUpdates.createOrUpdate.push({
        path: ClaudeDeployer.CLAUDE_PATH,
  ...
  /**
   * Get existing content from CLAUDE.md file
   */
  private async getExistingContent(gitRepo: GitRepo): Promise<string> {
    ...
    try {
      const existingFile = await this.gitHexa.getFileFromRepo(
        gitRepo,
        ClaudeDeployer.CLAUDE_PATH,
      );
      return existingFile?.content || '';
    } catch (error) {
      this.logger.debug('Failed to get existing Claude content', {
  ...
  private async generateStandardsContent(
    ...
    if (hasStandardsInstructions) {
      this.logger.debug(
        'Standards instructions already present in Claude content',
      );
      return existingContent;
    }
