...
describe('ClaudePluginDeployer', () => {
  ...
  describe('deployTrackingHooks', () => {
    ...
    describe('track-install.sh content', () => {
      ...
      it('reads the installed revision from the sidecar', () => {
        expect(shContent).toContain(
          'INSTALLED_REVISION="${PACKMIND_PLUGIN_REVISION:-}"',
        );
      });

      it('includes installedRevision in the payload', () => {
        expect(shContent).toContain('installedRevision');
      });
    ...
    describe('track-install.ps1 content', () => {
      ...
      it('includes installedRevision in the payload', () => {
        expect(ps1Content).toContain('installedRevision');
      });
    ...
    describe('token and name injection', () => {
      ...
      it('injects the content revision into the sidecar', () => {
        ...
        expect(env?.content).toContain(
          'PACKMIND_PLUGIN_REVISION=rev-deadbeef0001',
        );
      });
      ...
      describe('when marketplace name contains a newline (injection attempt)', () => {
        ...
        it('produces exactly 5 KEY=VALUE lines plus a trailing empty line', () => {
          // 5 KEY=VALUE lines + 1 trailing empty line = 6 lines
          expect(envLines).toHaveLength(6);
        });
      });
    });
  });
