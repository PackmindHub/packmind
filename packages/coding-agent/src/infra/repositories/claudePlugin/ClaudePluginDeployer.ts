...
export type PluginTrackingHooksInput = {
  ...
  /**
   * Content revision baked into the tracking sidecar so the SessionStart hook
   * can report it back, letting the marketplace classify installs as
   * up-to-date or outdated independently of the hardcoded plugin version.
   */
  revision: string;
};
...
function buildTrackInstallPs1(input: PluginTrackingHooksInput): string {
  ...
  return (
    ...
    `${S}InstalledRevision = ${S}PACKMIND_PLUGIN_REVISION\n` +
    '\n' +
    ...
    `if (${S}InstalledRevision) { ${S}Payload['installedRevision']    = ${S}InstalledRevision }\n` +
...
function buildTrackingEnv(input: PluginTrackingHooksInput): string {
  return [
    ...
    `PACKMIND_PLUGIN_REVISION=${sanitizeSidecarValue(input.revision)}`,
    '',
  ].join('\n');
}
