export const CREATE_ITEMS_0350 = `## Draft Format (CLI 0.35.0)
...
packmind playbook add --space $SPACE_SLUG .packmind/standards/_drafts/<slug>.md
...
packmind playbook add --space $SPACE_SLUG .packmind/commands/_drafts/<slug>.md
...
For each standard:
\`\`\`bash
packmind packages add --to @$SPACE_SLUG/$PACKAGE_SLUG --standard <slug>
\`\`\`

For each command:
\`\`\`bash
packmind packages add --to @$SPACE_SLUG/$PACKAGE_SLUG --command <slug>
\`\`\`
...
\`\`\`bash
packmind install @$SPACE_SLUG/$PACKAGE_SLUG
\`\`\`
