...
For each new skill that passes validation, write the skill directory following the format in [agent-skills-specification.md](agent-skills-specification.md). At minimum, create \`<skill-name>/SKILL.md\` with valid YAML frontmatter (\`name\`, \`description\`) and a markdown body. Move detailed reference material into \`references/\` files to keep SKILL.md under 500 lines.
