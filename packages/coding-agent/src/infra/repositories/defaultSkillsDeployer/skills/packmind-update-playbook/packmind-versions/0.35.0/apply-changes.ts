export const APPLY_CHANGES_0350 = `# Applying Changes

#### Pre-flight: Space Discovery

Before writing any files, discover available spaces:

1. Run \`packmind spaces list\` to see available spaces.
...
\`\`\`
packmind playbook add <path-to-the-file-you-edited>
\`\`\`

- If the organization has **multiple spaces** and this is a **new** artifact, add the \`--space\` flag: \`packmind playbook add <path> --space <slug>\`
- For **updates**, the space auto-resolves from the lock file — no \`--space\` needed.
- For **deprecated artifacts**: run \`packmind playbook rm <path>\` instead.

If any command fails, show the full error output, stop, and ask the user how to proceed — do not retry silently.

> **Mistake?** If you staged the wrong file, run \`packmind playbook unstage <path>\` to undo it before submitting.

##### Step 3: Review staged changes

Run \`packmind playbook status\` and present the output to the user. Verify:
...
Run \`packmind playbook submit -m "<intent description>"\` to submit all staged changes as proposals for human review.
...
Once every intent has been submitted, run \`packmind whoami\` and extract the \`Organization:\` field from the output. Construct the review URL as \`https://app.packmind.ai/org/<organization>/review-changes/\`.
