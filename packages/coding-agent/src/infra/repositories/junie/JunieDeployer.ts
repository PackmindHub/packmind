...
export class JunieDeployer implements ICodingAgentDeployer {
  private static readonly JUNIE_PATH = '.junie/guidelines.md';
  ...
  async deployRecipes(
    ...
    this.logger.info('Deploying recipes for Junie', {
      recipesCount: recipeVersions.length,
      gitRepoId: gitRepo.id,
    });

    // Get existing Junie content
  ...
  async deployStandards(
    ...
    this.logger.info('Deploying standards for Junie', {
  ...
  /**
   * Get existing content from .junie/guidelines.md file
   */
  private async getExistingContent(gitRepo: GitRepo): Promise<string> {
    ...
    try {
      const existingFile = await this.gitHexa.getFileFromRepo(
        gitRepo,
        JunieDeployer.JUNIE_PATH,
      );
      return existingFile?.content || '';
    } catch (error) {
      this.logger.debug('Failed to get existing Junie content', {
  ...
  private async generateStandardsContent(
    ...
    const standardsInstructions = `Follow the coding standards defined in [.packmind/standards-index.md](.packmind/standards-index.md)`;
    ...
    if (hasStandardsInstructions) {
      this.logger.debug(
        'Standards instructions already present in Junie content',
      );
      return existingContent;
    }
