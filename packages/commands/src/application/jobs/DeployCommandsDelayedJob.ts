...
import {
  DeployCommandsInput,
  DeployCommandsOutput,
} from '../../domain/jobs/DeployCommands';
...
export class DeployCommandsDelayedJob extends AbstractAIDelayedJob<
  DeployCommandsInput,
  DeployCommandsOutput
> {
  ...
  constructor(
    ...
    ) => Promise<IQueue<DeployCommandsInput, DeployCommandsOutput>>,
  ...
  getJobName(input: DeployCommandsInput): string {
    return `deploy-recipes-${input.gitRepoId}-${Date.now()}`;
  }

  jobStartedInfo(input: DeployCommandsInput): string {
    return `repo: ${input.gitRepoId}, versions: ${input.recipeVersionIds.length}`;
  }

  getWorkerListener(): Partial<
    WorkerListeners<DeployCommandsInput, DeployCommandsOutput>
  > {
    return {
      completed: async (
        job: Job<DeployCommandsInput, DeployCommandsOutput, string>,
        result: DeployCommandsOutput,
      ) => {
        this.logger.info(
