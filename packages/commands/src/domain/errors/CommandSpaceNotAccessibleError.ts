 ...
 *
 * Named `CommandSpaceNotAccessible` rather than `SpaceNotAccessible` because
 * `@packmind/deployments`, `@packmind/editions` and `@packmind/standards`
 * already export space not-found classes.
 */
export class CommandSpaceNotAccessibleError extends CommandsError {
  constructor(spaceId: string, organizationId?: string) {
    ...
    this.name = 'CommandSpaceNotAccessibleError';
  }
}
