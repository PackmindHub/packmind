...
export class PackageService {
  ...
  setArtefactPorts(ports: {
    ...
    this.commandsPort = ports.commandsPort;
   ...
   * `PackageWithArtefacts` by fetching commands/standards/skills through their
  ...
  private async enrichWithArtefacts(
    ...
    const commandsPort = this.requirePort(this.commandsPort, 'commandsPort');
    ...
    const uniqueCommandIds = [...new Set(packages.flatMap((p) => p.recipes))];
    const uniqueStandardIds = [
      ...new Set(packages.flatMap((p) => p.standards)),
    ];
    const uniqueSkillIds = [...new Set(packages.flatMap((p) => p.skills))];

    const [commands, standards, skills] = await Promise.all([
      uniqueCommandIds.length > 0
        ? commandsPort.getCommandsByIdsInternal(uniqueCommandIds)
