...
export default class TypeScriptTSXParser extends BaseParser {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const wasmPaths = BaseParser.getLanguageWasmPaths('tsx');
      ...
      if (!lang) {
        throw new Error(
          `Failed to load tree-sitter-tsx WASM file. Tried paths: ${wasmPaths.join(', ')}`,
        );
      }
      ...
      throw new ParserInitializationError(
        'tsx',
        'Failed to load TypeScript TSX WASM parser module',
        error as Error,
      );
    }
  }
  ...
  async parse(sourceCode: string): Promise<ASTNode> {
    ...
    if (!this.parser) {
      throw new ParserInitializationError('tsx', 'Parser not initialized');
    }

    const tree = this.parser.parse(sourceCode);
    if (!tree) {
      throw new ParserInitializationError('tsx', 'Failed to parse source code');
    }
