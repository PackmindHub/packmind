...
export function withSpan<T>(
  ...
  return tracer.startActiveSpan(name, async (span) => {
    try {
      ...
      // A denial is the service working correctly, so only mark the span as
      // failed when the error is not a domain error. This keeps the error ratio
      // useful for alerting instead of rising with ordinary permission denials.
