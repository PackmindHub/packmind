...
import { SkillsAdapterPortsMissingError } from './SkillsAdapterPortsMissingError';
import { SkillsHexaDependencyMissingError } from './SkillsHexaDependencyMissingError';
...
describe('SkillsAdapterPortsMissingError', () => {
  const error = new SkillsAdapterPortsMissingError([
    'ISpacesPort',
    'eventEmitterService',
  ]);
  ...
  it('answers its own reason', () => {
    expect(error.reason).toBe('skills_adapter_ports_missing');
  });
...
describe('SkillsHexaDependencyMissingError', () => {
  const error = new SkillsHexaDependencyMissingError(
  ...
  it('keeps the dependency in the context', () => {
    expect(error.context).toEqual({
      dependency: 'PackmindEventEmitterService',
    });
  });
});
