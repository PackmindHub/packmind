 ...
 *
 * Named `StandardSpaceNotAccessible` rather than `SpaceNotAccessible` because
 * `@packmind/deployments` and `@packmind/editions` already export space
 * not-found classes.
 */
export class StandardSpaceNotAccessibleError extends StandardsError {
  constructor(spaceId: string, organizationId?: string) {
    ...
    this.name = 'StandardSpaceNotAccessibleError';
  }
}
