...
while [[ -z "$CONTAINER_NAME" ]]; do
  ...
  if [[ -z "$CONTAINER_NAME" ]]; then
    if [[ $WAIT_COUNT -ge $MAX_WAIT ]]; then
      echo "[e2e] ERROR: No container matching '$CONTAINER_PATTERN' found within ${MAX_WAIT}s" >&2
      echo "[e2e] All containers:" >&2
...
while true; do
  ...
  case "$STATUS" in
    ...
    "created")
      if [[ $WAIT_COUNT -ge $MAX_WAIT ]]; then
        echo "[e2e] ERROR: Container stuck in '$STATUS' state for ${MAX_WAIT}s" >&2
        echo "[e2e] Container is waiting for dependencies. Docker compose status:" >&2
        docker compose ps 2>/dev/null || true
        echo "[e2e] Docker compose logs (last 100 lines):" >&2
    ...
    *)
      ...
      if [[ $WAIT_COUNT -ge $MAX_WAIT ]]; then
        echo "[e2e] ERROR: Timeout waiting for container, current status: $STATUS" >&2
        docker compose ps 2>/dev/null || true
        exit 1
      fi
      ;;
